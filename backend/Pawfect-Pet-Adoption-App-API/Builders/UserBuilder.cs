...
namespace Pawfect_Pet_Adoption_App_API.Builders
{
    ...
    public class UserBuilder : BaseBuilder<UserDto, User>
    {
        ...
        public UserBuilder(ShelterLookup shelterLookup, IShelterService shelterService)
        {
        ...
        private async Task<Dictionary<string, ShelterDto>> CollectShelters(List<User> users, List<string> shelterFields)
        {
            ...
            List<string> shelterIds = users.Select(x => x.ShelterId).Distinct().ToList();
            ...
            return users.ToDictionary(x => x.Id, x => shelterDtoMap[x.ShelterId]);
        }
    }
}
