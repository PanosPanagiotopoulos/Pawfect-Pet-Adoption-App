...
namespace Pawfect_Pet_Adoption_App_API.Services.FilterServices
{
    public class FileFilterBuilder : IFilterBuilder<Data.Entities.File, FileLookup>
    {
        ...
        public async Task<FilterDefinition<Data.Entities.File>> Build(FileLookup lookup)
        {
            ...
            // Define cache key
            String cacheKey = $"{typeof(Data.Entities.File).FullName}_{userId}_{lookup.GetHashCode()}";

            if (!_memoryCache.TryGetValue(cacheKey, out FilterDefinition<Data.Entities.File> cachedFilter)) return cachedFilter;
            ...
            FilterDefinition<Data.Entities.File> requestedFilters = await lookup.EnrichLookup(_queryFactory).ApplyFilters();
