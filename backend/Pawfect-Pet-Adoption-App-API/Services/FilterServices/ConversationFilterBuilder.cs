...
namespace Pawfect_Pet_Adoption_App_API.Services.FilterServices
{
    public class ConversationFilterBuilder : IFilterBuilder<Conversation, ConversationLookup>
    {
        ...
        public async Task<FilterDefinition<Conversation>> Build(ConversationLookup lookup)
        {
            ...
            // Define cache key
            String cacheKey = $"{typeof(Conversation).FullName}_{userId}_{lookup.GetHashCode()}";

            if (!_memoryCache.TryGetValue(cacheKey, out FilterDefinition<Conversation> cachedFilter)) return cachedFilter;
            ...
            FilterDefinition<Conversation> requestedFilters = await lookup.EnrichLookup(_queryFactory).ApplyFilters();
