...
namespace Pawfect_Pet_Adoption_App_API.Services.FilterServices
{
    public class NotificationFilterBuilder : IFilterBuilder<Notification, NotificationLookup>
    {
        ...
        public async Task<FilterDefinition<Notification>> Build(NotificationLookup lookup)
        {
            ...
            // Define cache key
            String cacheKey = $"{typeof(Notification).FullName}_{userId}_{lookup.GetHashCode()}";

            if (!_memoryCache.TryGetValue(cacheKey, out FilterDefinition<Notification> cachedFilter)) return cachedFilter;
            ...
            FilterDefinition<Notification> requestedFilters = await lookup.EnrichLookup(_queryFactory).ApplyFilters();
