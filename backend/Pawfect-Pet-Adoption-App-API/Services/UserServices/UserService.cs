...
namespace Pawfect_Pet_Adoption_App_API.Services.UserServices
{
		...
		public async Task<Boolean> VerifyUserAsync(String id, String email)
	{
		...
		{
			GoogleTokenResponse tokenResponse = await _authenticationService.ExchangeCodeForAccessToken(authorisationCode);
		...
		public async Task<Data.Entities.User> GetGoogleUser(String authorisationCode)
		{
		...
		{
			if (!await _authorisationService.AuthorizeOrOwnedAsync(_authorisationContentResolver.BuildOwnedResource(new UserLookup(), ids), Permission.DeleteUsers))
