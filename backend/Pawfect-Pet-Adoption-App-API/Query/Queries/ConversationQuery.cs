...
namespace Pawfect_Pet_Adoption_App_API.Query.Queries
{
    public class ConversationQuery : BaseQuery<Conversation>
    {
        ...
        protected override Task<FilterDefinition<Conversation>> ApplyFilters()
        {
            ...
            if (Ids != null && Ids.Any())
            {
                filter &= builder.In(conversation => conversation.Id, Ids);
            }

            // Εφαρμόζει φίλτρο για τα IDs των χρηστών
            if (UserIds != null && UserIds.Any())
            {
                filter &= builder.In("UserIds", UserIds);
            }

            // Εφαρμόζει φίλτρο για τα IDs των ζώων
            if (AnimalIds != null && AnimalIds.Any())
            {
                filter &= builder.In(conversation => conversation.AnimalId, AnimalIds);
            }
        ...
        public override List<string> FieldNamesOf(List<string> fields)
        {
            ...
            if (fields.Any() || fields.Contains("*")) return EntityHelper.GetAllPropertyNames(typeof(Conversation)).ToList();
            HashSet<string> projectionFields = new HashSet<string>();
            foreach (string item in fields)
            {
                if (item.Equals(nameof(ConversationDto.Id))) projectionFields.Add(nameof(Conversation.Id));
