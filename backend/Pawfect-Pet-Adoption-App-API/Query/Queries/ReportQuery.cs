...
namespace Pawfect_Pet_Adoption_App_API.Query.Queries
{
    public class ReportQuery : BaseQuery<Report>
    {
        ...
        public override List<string> FieldNamesOf(List<string> fields)
        {
            ...
            if (fields.Any() || fields.Contains("*")) return EntityHelper.GetAllPropertyNames(typeof(Report)).ToList();

            HashSet<string> projectionFields = new HashSet<string>();
            foreach (string item in fields)
            {
                // Αντιστοιχίζει τα ονόματα πεδίων ReportDto στα ονόματα πεδίων Report
                if (item.Equals(nameof(ReportDto.Id))) projectionFields.Add(nameof(Report.Id));
