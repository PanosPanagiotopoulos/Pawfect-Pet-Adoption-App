...
@Component({
  ...
  template: `
    <div class="flex items-center gap-3 p-3 sm:p-4 bg-gray-800/95 border-l-4 border-green-500 rounded-lg shadow-lg max-w-[90vw] sm:max-w-md">
      <!-- Icon -->
      <ng-icon
        [name]="data.icon ?? 'lucidePawPrint'"
        [size]="'20'"
        class="text-green-500 animate-pulse flex-shrink-0"
        aria-hidden="true"
      ></ng-icon>
      ...
      <div class="flex-1 min-w-0">
        <p class="text-sm font-medium text-white truncate">
          {{ data.message || ('APP.UI_COMPONENTS.SUCCESS_SNACKBAR.DEFAULT_MESSAGE' | translate) }}
        </p>
      ...
      <button
        ...
        [attr.aria-label]="'APP.UI_COMPONENTS.SUCCESS_SNACKBAR.CLOSE' | translate"
      >
        <ng-icon
          name="lucideX"
          [size]="'18'"
