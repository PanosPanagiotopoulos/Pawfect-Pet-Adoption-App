...
import { FormInputErrorTrackerService, ValidationErrorInfo } from 'src/app/common/services/form-input-error-tracker.service';
import { AuthService } from 'src/app/services/auth.service';
import { nameof } from 'ts-simple-nameof';

import { Animal } from 'src/app/models/animal/animal.model';
import { AdoptionApplication, AdoptionApplicationPersist, ApplicationStatus } from 'src/app/models/adoption-application/adoption-application.model';
    ...
    <div class="bg-white/5 backdrop-blur-lg rounded-2xl p-6 border border-white/10">
      <div class="mb-8 p-4">
        <div class="flex items-center space-x-4">
          <img [src]="animal.attachedPhotos?.[0]?.sourceUrl || '/assets/placeholder.jpg'" [alt]="animal.name" class="w-20 h-20 rounded-lg object-cover" />
          <div>
      ...
      <app-form-error-summary *ngIf="showErrorSummary" [errors]="validationErrors" [title]="'APP.ADOPT.ERROR_SUMMARY_TITLE' | translate"></app-form-error-summary>

      <form [formGroup]="applicationForm" (ngSubmit)="onSubmit()" #formContainer>
        <div class="space-y-6">
          <div *ngIf="isEditMode" class="mb-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            <div class="flex items-start space-x-3">
              <ng-icon name="lucideInfo" [size]="'20'" class="text-blue-400 mt-0.5 flex-shrink-0"></ng-icon>
              <div>
                <h4 class="text-blue-300 font-medium mb-1">{{ 'APP.ADOPT.STATUS_MANAGEMENT_INFO_TITLE' | translate }}</h4>
                <p class="text-blue-300/80 text-sm">{{ 'APP.ADOPT.STATUS_MANAGEMENT_INFO_MESSAGE' | translate }}</p>
              </div>
            </div>
          </div>

          <app-text-area-input [form]="applicationForm" controlName="applicationDetails" [label]="'APP.ADOPT.WHY_ADOPT_QUESTION' | translate" [rows]="4" [hint]="'APP.ADOPT.WHY_ADOPT_HINT' | translate"></app-text-area-input>

          <app-file-drop-area [form]="applicationForm" controlName="attachedFiles" [label]="'APP.ADOPT.SUPPORTING_DOCUMENTS' | translate" [hint]="'APP.ADOPT.SUPPORTING_DOCUMENTS_HINT' | translate" [multiple]="true" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" [maxFileSize]="10 * 1024 * 1024" [maxFiles]="5" [existingFiles]="existingFiles" (filesChange)="onFilesChange($event)"></app-file-drop-area>

          <div *ngIf="isEditMode && canManageApplicationStatus && isCurrentUserShelter" class="space-y-4">
            <div class="border-t border-white/10 pt-6">
              <h4 class="text-lg font-semibold text-white mb-4 flex items-center space-x-2">
                <ng-icon name="lucideSettings" [size]="'20'" class="text-primary-400"></ng-icon>
                <span>{{ 'APP.ADOPT.APPLICATION_STATUS_MANAGEMENT' | translate }}</span>
              </h4>

              <div class="mb-4">
                <label class="block text-sm font-medium text-gray-400 mb-2">{{ 'APP.ADOPT.CURRENT_STATUS' | translate }}</label>
                <div class="flex items-center space-x-3">
                  <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border" [ngClass]="getStatusChipClass(adoptionApplication?.status || ApplicationStatus.Pending)">
                    {{ getStatusTranslationKey(adoptionApplication?.status || ApplicationStatus.Pending) | translate }}
                  </span>
                </div>
              </div>

              <div class="space-y-3">
                <label class="block text-sm font-medium text-gray-400">{{ 'APP.ADOPT.UPDATE_STATUS' | translate }}</label>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div *ngFor="let option of statusOptions" class="relative">
                    <input type="radio" [id]="'status-' + option.value" [value]="option.value" formControlName="status" class="sr-only peer" />
                    <label [for]="'status-' + option.value" class="flex items-center justify-center p-3 rounded-lg border-2 cursor-pointer transition-all duration-200 border-white/20 bg-white/5 text-gray-300 hover:border-primary-500/50 hover:bg-primary-500/10 peer-checked:border-primary-500 peer-checked:bg-primary-500/20 peer-checked:text-white">
                      <span class="text-sm font-medium">{{ option.label | translate }}</span>
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div class="mt-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <div class="flex items-start space-x-2">
                  <ng-icon name="lucideBadgeAlert" [size]="'16'" class="text-amber-400 mt-0.5 flex-shrink-0"></ng-icon>
                  <p class="text-sm text-amber-300">{{ 'APP.ADOPT.STATUS_CHANGE_WARNING' | translate }}</p>
                </div>
              </div>
            </div>
          </div>
...
export class AdoptionFormComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['adoptionApplication'] && !changes['adoptionApplication'].firstChange) {
      this.loadExistingData();
    }
  }
  ...
  onSubmit() {
    ...
    if (this.applicationForm.valid) {
      ...
      this.adoptionApplicationService.persist(application, this.getPersistFields()).subscribe({
      ...
      this.validationErrors = this.formErrorTracker.trackFormErrors(this.applicationForm, this.formContainer.nativeElement);
  ...
  private initializePermissions() {
    this.canManageApplicationStatus = this.authService.hasPermission(Permission.EditAdoptionApplications);

    if (this.isEditMode && this.adoptionApplication && this.animal?.shelter) {
      const currentUserEmail = this.authService.getUserEmail();
      this.isCurrentUserShelter = currentUserEmail === this.animal.shelter.user?.email;
    }
  ...
  private determineStatusToUse(): ApplicationStatus {
    if (this.isEditMode && this.canManageApplicationStatus && this.isCurrentUserShelter) {
      return this.applicationForm.get('status')?.value || ApplicationStatus.Pending;
    } else {
  ...
  private buildApplicationPersist(statusToUse: ApplicationStatus): AdoptionApplicationPersist {
    return {
      id: this.isEditMode ? this.adoptionApplication?.id || '' : '',
      animalId: this.isEditMode ? this.adoptionApplication?.animal?.id! : this.animal.id!,
      shelterId: this.isEditMode ? this.adoptionApplication?.shelter?.id! : this.animal.shelter?.id!,
      status: statusToUse,
