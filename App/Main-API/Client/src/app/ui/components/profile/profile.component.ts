...
import { User, UserPersist } from 'src/app/models/user/user.model';
  ...
  implements OnInit, OnDestroy
{
  ...
  private initializeOperatingHours(): void {
    ...
    this.days.forEach((day) => {
      ...
      const value = this.profileUser?.shelter?.operatingHours?.[dayKey as keyof typeof this.profileUser.shelter.operatingHours];
  ...
  private createEditForm(): void {
    ...
    this.editForm = this.fb.group({
      // Only profile photo for personal info
      profilePhoto: [null],
  ...
  async saveChanges(): Promise<void> {
    ...
    try {
      ...
      const profilePhotoFile = this.editForm.get('profilePhoto')?.value;
      const shouldDeletePhoto = this.editForm.get('deletePhoto')?.value;
      
      if (profilePhotoFile || shouldDeletePhoto) {
        // TODO: Implement actual file upload/delete logic
        // For now, just show a message about what would happen
        if (shouldDeletePhoto) {
          console.log('Profile photo would be deleted');
          this.snackbarService.showSuccess({
            message: this.translationService.translate('APP.PROFILE-PAGE.EDIT.PHOTO_DELETE_PLACEHOLDER')
          });
        } else if (profilePhotoFile) {
          console.log('Profile photo would be uploaded:', profilePhotoFile.name);
          this.snackbarService.showSuccess({
            message: this.translationService.translate('APP.PROFILE-PAGE.EDIT.PHOTO_UPLOAD_PLACEHOLDER')
          });
        }
      }

      // Save shelter changes (if user is a shelter)
      if (
        isShelter &&
        ...
        const shelterPersist: ShelterPersist = {
          ...
          socialMedia: {
            facebook:
              socialMediaForm?.get('facebook')?.value ||
              this.profileUser.shelter.socialMedia?.facebook ||
              '',
            instagram:
              socialMediaForm?.get('instagram')?.value ||
              this.profileUser.shelter.socialMedia?.instagram ||
              '',
          },
          operatingHours: {
            monday:
              operatingHoursForm?.get('monday')?.value ||
              this.profileUser.shelter.operatingHours?.monday ||
              '',
            tuesday:
              operatingHoursForm?.get('tuesday')?.value ||
              this.profileUser.shelter.operatingHours?.tuesday ||
              '',
            wednesday:
              operatingHoursForm?.get('wednesday')?.value ||
              this.profileUser.shelter.operatingHours?.wednesday ||
              '',
            thursday:
              operatingHoursForm?.get('thursday')?.value ||
              this.profileUser.shelter.operatingHours?.thursday ||
              '',
            friday:
              operatingHoursForm?.get('friday')?.value ||
              this.profileUser.shelter.operatingHours?.friday ||
              '',
            saturday:
              operatingHoursForm?.get('saturday')?.value ||
              this.profileUser.shelter.operatingHours?.saturday ||
              '',
            sunday:
              operatingHoursForm?.get('sunday')?.value ||
              this.profileUser.shelter.operatingHours?.sunday ||
              '',
          },
          verificationStatus:
        ...
        const updatedShelter = await this.shelterService
          .persist(shelterPersist, this.shelterFields)
          .toPromise();
        if (updatedShelter && this.profileUser.shelter) {
          this.profileUser.shelter = updatedShelter;
        }
      }
  ...
  openProfilePhotoDialog(): void {
    ...
    const dialogData: ProfilePhotoDialogData = {
      currentPhotoUrl: this.profileUser.profilePhoto?.sourceUrl,
    ...
    dialogRef.afterClosed().subscribe((result: ProfilePhotoDialogResult) => {
      if (result) {
        switch (result.action) {
          case 'upload':
            if (result.file) {
              this.editForm?.get('profilePhoto')?.setValue(result.file);
              this.snackbarService.showSuccess({
                message: this.translationService.translate('APP.PROFILE-PAGE.EDIT.PHOTO_SELECTED')
              });
            }
            break;
          case 'delete':
            this.editForm?.get('profilePhoto')?.setValue(null);
            // Set a flag to indicate photo should be deleted
            this.editForm?.get('deletePhoto')?.setValue(true);
            this.snackbarService.showSuccess({
              message: this.translationService.translate('APP.PROFILE-PAGE.EDIT.PHOTO_WILL_BE_DELETED')
            });
            break;
          case 'cancel':
            // Do nothing
