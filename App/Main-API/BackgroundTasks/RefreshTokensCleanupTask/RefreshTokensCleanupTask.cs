            ...
            using (IServiceScope serviceScope = _serviceProvider.CreateScope())
            {
                ...
                while (true)
                {
                    using (IClientSessionHandle session = await mongoClient.StartSessionAsync())
                    {
                        ...
                        try
                        {
                            List<Main_API.Data.Entities.RefreshToken> tokensToCleanup = 
                                await refreshTokenRepository.FindManyAsync(
                                    rToken => rToken.ExpiresAt >= DateTime.UtcNow, 
                                    new List<String>() { nameof(Main_API.Data.Entities.RefreshToken.Id) }, 
                                    session
                            );
                        ...
                        {
                            _logging.LogError(ex, $"Could not clean up of type: {nameof(Main_API.Data.Entities.File)}");
