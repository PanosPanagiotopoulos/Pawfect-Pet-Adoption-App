...
namespace Pawfect_API.Services.AnimalServices
{
	public class AnimalService : IAnimalService
	{
        ...
        public async Task<List<Models.Animal.Animal>> PersistBatch(List<AnimalPersist> models, List<String> fields)
        {
            ...
            List<Data.Entities.Animal> adoptedAnimals = new List<Data.Entities.Animal>();
            ...
            if (isUpdate)
            {
                ...
                adoptedAnimals = datas.Where(a => a.AdoptionStatus == AdoptionStatus.Adopted).ToList();
            }
            else
            {
            ...
            // Delete adoption requests for adopted animals & send notification to users that tried to adopt these animals
            if (adoptedAnimals != null && adoptedAnimals.Count > 0) await this.HandleAdoptedAnimals(adoptedAnimals);
        ...
        #region Handle Adopted Animals
        private async Task HandleAdoptedAnimals(List<Data.Entities.Animal> adoptedAnimals)
        {
            List<String> adoptedAnimalIds = adoptedAnimals.Select(x => x.Id).ToList();

            AdoptionApplicationLookup applicationsToRejectLookup = new AdoptionApplicationLookup();
            applicationsToRejectLookup.AnimalIds = adoptedAnimalIds;
            applicationsToRejectLookup.ShelterIds = new List<String>() { await _authorizationContentResolver.CurrentPrincipalShelter() };
            applicationsToRejectLookup.Offset = 0;
            applicationsToRejectLookup.PageSize = 100000;

            List<Data.Entities.AdoptionApplication> toRejectApplications = await applicationsToRejectLookup.EnrichLookup(_queryFactory).CollectAsync();
            toRejectApplications.ForEach(app => { app.Status = ApplicationStatus.Rejected; app.UpdatedAt = DateTime.UtcNow; });
            if ((await _adoptionApplicationRepository.UpdateManyAsync(toRejectApplications)).Count != toRejectApplications.Count) throw new Exception("Failed to reject connect applications to these animals");
           
            List<String> applicationsUserIds = toRejectApplications.Select(app => app.UserId).Distinct().ToList();
            // Notify users that their applications got rejected
            List<NotificationEvent> events = new List<NotificationEvent>();

            Data.Entities.Shelter shelter = await _shelterRepository.FindAsync(s => s.Id == toRejectApplications.FirstOrDefault().ShelterId, new List<String> { nameof(Data.Entities.Shelter.ShelterName) });
            if (shelter == null) throw new NotFoundException("Shelter to send notification from not found");

            Dictionary<String, String> animalNames = (await _animalRepository.FindManyAsync(a =>  adoptedAnimalIds.Contains(a.Id), new List<String> { nameof(Data.Entities.Animal.Id), nameof(Data.Entities.Animal.Name) })).ToDictionary(x => x.Id, x => x.Name );
            if (animalNames == null || !animalNames.Any()) throw new NotFoundException("Refering animal from rejected applications not found");
            ...
            foreach (Data.Entities.AdoptionApplication app in toRejectApplications)
            {
                String animalName = animalNames.ContainsKey(app.AnimalId) ? animalNames[app.AnimalId] : "Animal";
                ...
                NotificationEvent ev = new NotificationEvent
                {
                    ...
                    ContentMappings = new Dictionary<String, String>
                    {
                        ...
                        { _notificationConfig.AdoptionApplicationChangedUserPlaceholders.AnimalName, animalName },
