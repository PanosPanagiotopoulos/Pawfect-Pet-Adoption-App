    ...
    public class ExcelExtractor : IFileDataExtractor
    {
        ...
        public async Task<Byte[]> GenerateAnimalImportTemplate()
        {
            ...
            foreach (String header in headers)
            {
                ...
                else if (String.Equals(header, String.Join(' ', nameof(Pawfect_API.Models.Animal.Animal.AnimalType), nameof(Pawfect_API.Models.AnimalType.AnimalType.Name)), StringComparison.OrdinalIgnoreCase))
                {
                ...
                else if (String.Equals(header, String.Join(' ', nameof(Pawfect_API.Models.Animal.Animal.Breed), nameof(Pawfect_API.Models.Breed.Breed.Name)), StringComparison.OrdinalIgnoreCase))
                {
        ...
        private async Task<List<(String Id, String Name, String AnimalTypeId)>> CollectBreedColumnSetup()
        {
            ...
            q.Offset = 0;
            q.PageSize = 1000;
            q.Fields = q.FieldNamesOf(new List<String> { nameof(Pawfect_API.Models.Breed.Breed.Id), nameof(Pawfect_API.Models.Breed.Breed.Name), $"{nameof(Pawfect_API.Models.Breed.Breed.AnimalType)}.{nameof(Pawfect_API.Models.AnimalType.AnimalType.Id)}" });

            List<Pawfect_API.Data.Entities.Breed> breeds = await q.CollectAsync();
        ...
        private async Task<List<(String Id, String Name)>> CollectAnimalTypesColumnSetup()
        {
            ...
            q.Offset = 0;
            q.PageSize = 1000;
            q.Fields = q.FieldNamesOf(new List<String> { nameof(Pawfect_API.Models.AnimalType.AnimalType.Id), nameof(Pawfect_API.Models.AnimalType.AnimalType.Name) });

            List<Pawfect_API.Data.Entities.AnimalType> types = await q.CollectAsync();
        ...
        public async Task<List<AnimalPersist>> ExtractAnimalModelData(IFormFile modelsDataCsv)
        {
            ...
            for (int rowIdx = startRow; rowIdx <= totalRows; rowIdx++)
            {
                ...
                String breedName = sheet.Cells[rowIdx, headerMap.GetValueOrDefault(String.Join(' ', nameof(Pawfect_API.Models.Animal.Animal.Breed), nameof(Pawfect_API.Models.Breed.Breed.Name)))].Text;
                String typeName = sheet.Cells[rowIdx, headerMap.GetValueOrDefault(String.Join(' ', nameof(Pawfect_API.Models.Animal.Animal.AnimalType), nameof(Pawfect_API.Models.AnimalType.AnimalType.Name)))].Text;
