...
namespace Pawfect_Messenger.Query.Queries
{
	public class UserQuery : BaseQuery<Data.Entities.User>
	{
        ...
        public UserQuery Authorise(AuthorizationFlags authorise) { _authorise = authorise; return this; }
        ...
        public override Task<FilterDefinition<Data.Entities.User>> ApplyFilters()
		{
			...
			{
				filter &= builder.Regex(user => user.FullName, new BsonRegularExpression(Query, "i"));
			}
			...
			{
                filter &= builder.AnyIn(user => user.Roles, Roles);
			}
            ...
            if (!String.IsNullOrEmpty(Query))
            {
                ...
                if (Query.Length >= 3)
                {
                    ...
                    for (int i = 0; i < escapedFuzzyQuery.Length; i++)
                    {
