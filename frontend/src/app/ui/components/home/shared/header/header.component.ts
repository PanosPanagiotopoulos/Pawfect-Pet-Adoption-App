...
@Component({
  ...
  template: `
    <nav class="fixed w-full top-0 z-50">
      ...
      <div class="absolute inset-0 bg-gradient-to-r from-gray-900/95 via-gray-800/95 to-gray-900/95 animate-gradient backdrop-blur-lg"></div>
        ...
        <div class="flex justify-between h-20">
          ...
          <div class="hidden md:flex items-center space-x-8">
            ...
            <ng-container *ngIf="!isLoggedIn; else userMenu">
              <app-auth-button routerLink="/auth/login">Login</app-auth-button>
            </ng-container>

            <ng-template #userMenu>
              <app-dropdown [(isOpen)]="isUserMenuOpen">
                <div trigger class="flex items-center space-x-2 cursor-pointer">
                  <app-user-avatar [imageUrl]="currentUser?.photoUrl" [name]="currentUser?.name"></app-user-avatar>
                </div>

                <app-dropdown-item routerLink="/profile" icon="lucideUser" (click)="isUserMenuOpen = false">Profile</app-dropdown-item>
                <app-dropdown-item (click)="logout()" icon="lucideLogOut">Logout</app-dropdown-item>
              </app-dropdown>
            </ng-template>
          </div>
